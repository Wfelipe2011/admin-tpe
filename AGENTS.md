...
- **Two API endpoints** (`legacy` | `new`) are wired in `lib/api-client.ts` via a shared `ApiClient`. Default is `legacy`; pass `{ endpoint: "new" }` on upload/newer endpoints. The `petitionApi` helper uses `new`.
